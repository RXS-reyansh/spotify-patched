...
var _API_instances, getTracks_fn, getPaginatedItems_fn;
var _API = class _API {
  constructor(clientId, clientSecret, topTracksCountry) {
    __privateAdd(this, _API_instances);
  ...
  async getData(url) {
    ...
    try {
      ...
      return {
        ...
        tracks: (await __privateMethod(this, _API_instances, getTracks_fn).call(this, body)).filter((t) => t?.type === "track")
      };
    } catch (e) {
      throw apiError(e);
    }
...
getTracks_fn = /* @__PURE__ */ __name(async function(data) {
  switch (data.type) {
    ...
    case "album": {
      return await __privateMethod(this, _API_instances, getPaginatedItems_fn).call(this, data);
    }
    case "playlist": {
      return (await __privateMethod(this, _API_instances, getPaginatedItems_fn).call(this, data)).map((i) => i.track).filter(import_distube.isTruthy);
    }
  }
}, "#getTracks");
...
var _SpotifyPlugin = class _SpotifyPlugin extends import_distube2.CustomPlugin {
  ...
  async play(voiceChannel, url, options) {
    ...
    if (data.type === "track") {
      ...
      if (!result) throw new import_distube2.DisTubeError("SPOTIFY_PLUGIN_NO_RESULT", `Cannot find "${query}" on YouTube.`);
